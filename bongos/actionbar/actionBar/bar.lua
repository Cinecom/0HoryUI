...
local function ShowMenu(bar)
	HoryUI.ShowBarMenu(bar, {
		title = "Action Bar " .. bar.id,
		size = {
			min = 1,
			max = function() return math.floor(MAX_BUTTONS / BActionBar.GetNumber()) end,
			step = 1,
			get = function() return BActionBar.GetSize(bar.id) end,
			set = function(v) Layout(bar.id, bar.sets.rows, bar.sets.space, v) end,
		},
		rows = {
			min = 1,
			max = function() return BActionBar.GetSize(bar.id) end,
			step = 1,
			get = function() return bar.sets.rows or DEFAULT_ROWS end,
			set = function(v) Layout(bar.id, v, bar.sets.space, bar.sets.size) end,
		},
		spacing = {
			min = 0, max = 36, step = 2,
			get = function() return bar.sets.space or DEFAULT_SPACING end,
			set = function(v) Layout(bar.id, bar.sets.rows, v, bar.sets.size) end,
		},
	})
end
