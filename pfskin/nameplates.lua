...
pfSkin:RegisterModule("nameplates", "vanilla", function ()
  ...
  local function GetCastInfo(guid)
    return pfSkin.libdebuff_casts and pfSkin.libdebuff_casts[guid]
  end
  ...
  local function CacheConfig()
    -- HoryUI nameplate layout overrides. Standalone pfSkin has no config GUI, so
    -- these are forced here (the single config-refresh point) rather than via a
  ...
  local er, eg, eb, ea = GetStringColor(pfSkin_config.appearance.border.color)
  ...
  local function GetCombatStateColor(guid)
    ...
    elseif cfg.ccombatofftank and pfSkin.uf and pfSkin.uf.raid and targetName and pfSkin.uf.raid.tankrole[targetName] then
      color = combatstate.OFFTANK
  ...
  local function GetNameString(name)
    local abbrev = pfSkin_config.unitframes.abbrevname == "1" or nil
    local size = 20
  ...
  local function CreateDebuffIcon(plate, index)
    ...
    -- HoryUI tweak: countdown number shown ONLY for short auras (< 60s) -- see the
    -- debuff display loop in OnDataChanged. Lives on the debuff frame's OVERLAY
    -- (above the icon); the cd swirl is hidden by default so this is the timer cue.
    plate.debuffs[index].timer = plate.debuffs[index]:CreateFontString(nil, "OVERLAY")
    plate.debuffs[index].timer:SetPoint("CENTER", plate.debuffs[index], "CENTER", 0, 0)
    plate.debuffs[index].timer:SetTextColor(1, .82, 0)
    ...
    if pfSkin.client <= 11200 and cfg.debuffanim ~= 1 then
  ...
  local function UpdateDebuffConfig(nameplate, i)
    ...
    local font = C.nameplates.use_unitfonts == "1" and pfSkin.font_unit or pfSkin.font_default
    ...
    if nameplate.debuffs[i].timer then nameplate.debuffs[i].timer:SetFont(font, font_size, font_style) end
...
if GetUnitField then
  ...
  pfSkin.libdebuff_on_unit_updated = pfSkin.libdebuff_on_unit_updated or {}
  table.insert(pfSkin.libdebuff_on_unit_updated, function(guid)
    local plate = guidRegistry[guid]
    if plate and plate.nameplate then
      plate.nameplate.auraUpdate = true
    end
      ...
      if plate:IsVisible() then
        ...
        if guid then
          ...
          if castInfo and castInfo.endTime and castInfo.endTime < frameState.now then
            if pfSkin.libdebuff_casts then
              pfSkin.libdebuff_casts[guid] = nil
            end
          end
  ...
  nameplates.OnCreate = function(frame)
    ...
    -- create pfSkin nameplate overlay
    ...
    nameplate.glow:SetTexture(pfSkin.media["img:dot"])
    nameplate.glow:Hide()
    ...
    if C.unitframes.blizzard_raidicons ~= "1" then
      nameplate.raidicon:SetTexture(pfSkin.media["img:raidicons"])
    end
  ...
  nameplates.OnConfigChange = function(frame)
    ...
    local hptexture = pfSkin.media[C.nameplates.healthtexture]
    local rawborder, default_border = GetBorderSize("nameplates")

    -- HoryUI: the name hangs BELOW the health bar in a much smaller font
    local np_name_size = font_size - 4
    if np_name_size < 7 then np_name_size = 7 end

    local plate_width = C.nameplates.width + 50
    local plate_height = C.nameplates.heighthealth + np_name_size + 5
    ...
    -- HoryUI: health bar pinned to the top of the plate; the name hangs beneath it
    ...
    -- HoryUI: castbar drops below the name so it never overlaps it
    local castdrop = np_name_size + 6
    nameplate.castbar:SetPoint("TOPLEFT", nameplate.health, "BOTTOMLEFT", 0, -castdrop)
    nameplate.castbar:SetPoint("TOPRIGHT", nameplate.health, "BOTTOMRIGHT", 0, -castdrop)
    ...
    local cbtexture = pfSkin.media[C.appearance.castbar.texture]
  ...
  nameplates.OnDataChanged = function(self, plate)
    ...
    elseif HidePlate(unittype, name, (hpmax-hp == hpmin), target) then
      ...
      -- HoryUI: level sits to the RIGHT of the bar; name (and guild) hang beneath it
      plate.level:ClearAllPoints()
      plate.level:SetPoint("LEFT", plate.health, "RIGHT", 5, 0)
    ...
    if cfg.showhp then
      ...
      if not rhp then
        if hpmax > 100 or (round(hpmax/100*hp) ~= hp) then
          rhp, rhpmax = hp, hpmax
        elseif pfSkin.libhealth and pfSkin.libhealth.enabled then
          rhp, rhpmax, estimated = pfSkin.libhealth:GetUnitHealthByName(name,level,tonumber(hp),tonumber(hpmax))
        end
      end
    ...
    -- HoryUI: hostile plates read as red -- the current target is a vivid red,
    -- every other plate desaturates so the target pops at a glance.
    if unittype == "ENEMY_NPC" or unittype == "ENEMY_PLAYER" then
      if target then
        r, g, b, a = 0.86, 0.16, 0.20, 1
      else
        r, g, b, a = 0.46, 0.25, 0.27, 1
      end
    end
  ...
  nameplates.OnShow = function(frame)
    ...
    if guid then
      ...
      if pfSkin.api.libunitscan and pfSkin.api.libunitscan.ScanGuid then
        local name = frame.nameplate.original.name:GetText()
        local npcFlags = GetUnitField(guid, "npcFlags") or 0
        pfSkin.api.libunitscan.ScanGuid(guid, name, npcFlags == 0)
      end
    end
  ...
  nameplates.OnUpdate = function(frame, state)
    ...
    local throttle
    if target then
      throttle = pfSkin.throttle:Get("nameplates_target")
    elseif visiblePlateCount > 20 then
      throttle = pfSkin.throttle:Get("nameplates_mass")
    else
      throttle = pfSkin.throttle:Get("nameplates")
    end

    -- Non-target plates with active castbar use the castbar throttle
    if isCastingNonTarget then
      local cbThrottle = pfSkin.throttle:Get("nameplates_castbar")
    ...
    if cfg.showcastbar and not cfg.targetcastbar and not isTargetPlate then
      ...
      if visiblePlateCount > 20 then
        local massThrottle = pfSkin.throttle:Get("nameplates_mass")
  ...
  nameplates.castbarFrame:SetScript("OnUpdate", function()
    ...
    local now = GetTime()
    local throttle = pfSkin.throttle:Get("nameplates_castbar")
