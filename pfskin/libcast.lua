...
--[[ libcast ]]--
-- A pfSkin library that detects and saves all ongoing castbars of players, NPCs and enemies.
...
--     Adds a spell to the database by using pfSkin's spell database
...
if pfSkin.api.libcast then return end
...
pfGetChannelInfo = function(unit)
  ...
  local db = nil
  if guid and pfSkin.libdebuff_casts and pfSkin.libdebuff_casts[guid] then
    -- Use libdebuff's cast tracking (from SPELL_START_OTHER events)
    local castData = pfSkin.libdebuff_casts[guid]
...
pfSkin.libdebuff_spell_start_self_hooks["libcast_icon"] = function(spellId)
  lastSpellId = spellId
end

-- add libcast to pfSkin API
pfSkin.api.libcast = libcast