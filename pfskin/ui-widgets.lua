...
do -- statusbars
  ...
  animate:SetScript("OnUpdate", function()
    stepsize = tonumber(pfSkin_config.unitframes.animation_speed)
  ...
  local handlers = {
    ["DisplayValue"] = function(self, val)
      ...
      if self.mode == "vertical" then
        ...
        if pfSkin.expansion == "vanilla" then height = height / self:GetEffectiveScale() end
        ...
        width = self:GetWidth()
        if pfSkin.expansion == "vanilla" then width = width / self:GetEffectiveScale() end
  ...
  function pfSkin.api.CreateStatusBar(name, parent)
...
do -- dropdown
  ...
  local handlers = {
    ...
    ["CreateMenuEntry"] = function(self, id)
      ...
      if not frame and not entry then
        ...
        frame.text:SetFont(pfSkin.font_default, pfSkin_config.global.font_size-1, "OUTLINE")
  ...
  function pfSkin.api.CreateDropDownButton(name, parent)
    ...
    text:SetFont(pfSkin.font_default, pfSkin_config.global.font_size-1, "OUTLINE")
...
function pfSkin.api.CreateTabChild(self, title, bwidth, bheight, bottom, static)
  -- create tab button
...
function pfSkin.api.CreateTabFrame(parent, align, outside)
  ...
  -- Create Child Frame
  f.CreateTabChild = pfSkin.api.CreateTabChild

  return f
end

function pfSkin.api.CreateScrollFrame(name, parent)
  ...
  f.slider:SetThumbTexture(pfSkin.media["img:col"])
...
function pfSkin.api.CreateScrollChild(name, parent)
...
-- [ CreateTextBox ]
-- Creates and returns a default pfSkin skinned EditBox
function pfSkin.api.CreateTextBox(name, parent)
...
function pfSkin.api.EnableClickRotate(frame)
...
function pfSkin.api.SetHighlight(frame, cr, cg, cb)
  ...
  frame.rr, frame.rg, frame.rb, frame.ra = GetStringColor(pfSkin_config.appearance.border.color)
...
function pfSkin.api.HandleIcon(frame, icon)
...
-- [ Skin Button ]
-- Applies pfSkin skin to buttons:
...
function pfSkin.api.SkinButton(button, cr, cg, cb, icon, disableHighlight)
  local b = _G[button]
  ...
  pfSkin.api.CreateBackdrop(b, nil, true)
  b:SetNormalTexture("")
  ...
  b:SetFont(pfSkin.font_default, pfSkin_config.global.font_size, "OUTLINE")
  ...
  b.UnlockHighlight = function()
    if not MouseIsOver(b) then
      b:SetBackdropBorderColor(GetStringColor(pfSkin_config.appearance.border.color))
    end
    b.locked = false
  end
end
...
-- Applies pfSkin skin to collapse/expand buttons:
-- 'button'   [frame/string]  the button that should be skinned.
function pfSkin.api.SkinCollapseButton(button, all)
  local b = _G[button]
  if not b then b = button end
  if not b then return end
...
-- [ Skin Rotate Button]
-- Applies pfSkin skin to rotation buttons like in character pane:
-- 'button'     [frame/string]  the button that should be skinned.
function pfSkin.api.SkinRotateButton(button)
  pfSkin.api.CreateBackdrop(button)
...
-- [ Skin Close Button ]
-- Applies pfSkin close skin to buttons and can also be positioned
...
function pfSkin.api.SkinCloseButton(button, parentFrame, offsetX, offsetY)
  ...
  button.texture:SetTexture(pfSkin.media["img:close"])
  button.texture:ClearAllPoints()
  button.texture:SetAllPoints(button)
  button.texture:SetVertexColor(1,.25,.25,1)
end

function pfSkin.api.SkinArrowButton(button, dir, size)
  if not button then return end
  ...
  button.icon:SetTexture(pfSkin.media["img:"..dir])
...
function pfSkin.api.SkinScrollbar(frame, always)
  ...
  pfSkin.api.SkinArrowButton(up, "up")
  pfSkin.api.SkinArrowButton(down, "down")
...
function pfSkin.api.CenterFrame(frame, relativeFrame)
  frame:ClearAllPoints()
...
function pfSkin.api.StripTextures(frame, hide, layer)
  if not frame then return end
...
function pfSkin.api.SkinDropDown(frame, cr, cg, cb, useSmall)
  if not frame then return end
  StripTextures(frame)
  CreateBackdrop(frame)
  ...
  if not button.icon then
    ...
    button.icon:SetTexture(pfSkin.media["img:down"])
    button.icon:SetVertexColor(1,.9,.1)
    button.icon:SetAlpha(.8)
...
function pfSkin.api.SkinTab(frame, fixed)
  frame:SetHeight(20)
  StripTextures(frame)
  CreateBackdrop(frame)
...
function pfSkin.api.SkinSlider(frame)
...
-- [ Question Dialog ]
-- Creates a pfSkin user dialog popup:
...
function pfSkin.api.CreateQuestionDialog(text, yes, no, editbox, onclose)
  ...
  pfSkin.api.CreateBackdrop(question, nil, nil, .85)
  pfSkin.api.CreateBackdropShadow(question)
  ...
  if editbox then
    ...
    pfSkin.api.CreateBackdrop(question.input)
  ...
  pfSkin.api.SkinButton(question.yes)
  ...
  pfSkin.api.SkinButton(question.no)
  ...
  pfSkin.api.CreateBackdrop(question.close)
  ...
  question.close.texture:SetTexture(pfSkin.media["img:close"])
  ...
  question.close:SetScript("OnLeave", function ()
    pfSkin.api.CreateBackdrop(this)
  end)
...
-- [ Question Dialog ]
-- Creates a pfSkin infobox popup window:
-- 'text'       [string]        text that will be displayed.
...
function pfSkin.api.CreateInfoBox(text, time, parent, height)
  ...
  if not infobox then
    ...
    infobox.timeout:SetStatusBarTexture(pfSkin.media["img:bar"])
    infobox.timeout:SetStatusBarColor(.3,1,.8,1)

    infobox:ClearAllPoints()
    infobox.text:SetAllPoints(infobox)
    infobox.text:SetFont(pfSkin.font_default, 14, "OUTLINE")

    pfSkin.api.CreateBackdrop(infobox)
...
function pfSkin.api.SkinMoneyInputFrame(frame)
