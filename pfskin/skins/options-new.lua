...
pfSkin:RegisterSkin("Options - New", "vanilla", function ()
  if not OptionsFrame or not OptionsFrameCategoryList then return end
  local rawborder, border = GetBorderSize()
  local bpad = rawborder > 1 and border - GetPerfectPixel() or GetPerfectPixel()
  local br, bg, bb, ba = pfSkin.api.GetStringColor(pfSkin_config.appearance.border.background)
  local er, eg, eb, ea = pfSkin.api.GetStringColor(pfSkin_config.appearance.border.color)
  ...
  if OptionsFrameCategoryList then
    ...
    catBG:SetBackdrop(pfSkin.backdrop)
  ...
  if OptionsFrameContent then
    ...
    contBG:SetBackdrop(pfSkin.backdrop)
  ...
  local function SkinControls()
    ...
    for i = 1, 20 do
      ...
      if not btn._pfSkinned then
        ...
        btn:SetBackdrop(pfSkin.backdrop)
        local br, bg, bb = pfSkin.api.GetStringColor(pfSkin_config.appearance.border.background)
        local er, eg, eb = pfSkin.api.GetStringColor(pfSkin_config.appearance.border.color)
        ...
        btn:SetFont(pfSkin.font_default, pfSkin_config.global.font_size, "OUTLINE")
        HookScript(btn, "OnMouseDown", function()
          StripTextures(this, true)
          this:SetBackdrop(pfSkin.backdrop)
