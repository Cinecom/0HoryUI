...
function libthrottle:Get(category)
  local configValue = _G.pfSkin_throttle and _G.pfSkin_throttle[category]
  ...
  if configValue == "custom" then
    local customFps = tonumber(_G.pfSkin_throttle[category .. "_custom"])
    if customFps then
...
function libthrottle:Set(category, value)
  if not _G.pfSkin_throttle then _G.pfSkin_throttle = {} end
  
  -- Validate preset
  if type(value) == "string" and self.presets[value] then
    _G.pfSkin_throttle[category] = value
    return true
  end
...
function libthrottle:ResetToDefault(category)
  ...
  if default then
    if not _G.pfSkin_throttle then _G.pfSkin_throttle = {} end
    _G.pfSkin_throttle[category] = default
    return true
  end
  return false
end
...
libthrottle:SetScript("OnEvent", function()
  if event == "PLAYER_ENTERING_WORLD" then
    if not _G.pfSkin_throttle then
      _G.pfSkin_throttle = {}
    end
    
    -- Set defaults for any missing categories
    for category, default in pairs(libthrottle.defaults) do
      if not _G.pfSkin_throttle[category] then
        _G.pfSkin_throttle[category] = default
      end
    end
    
    -- Set defaults for custom fields if missing
    if not _G.pfSkin_throttle.nameplates_target_custom then _G.pfSkin_throttle.nameplates_target_custom = "50" end
    if not _G.pfSkin_throttle.nameplates_custom then _G.pfSkin_throttle.nameplates_custom = "10" end
    if not _G.pfSkin_throttle.nameplates_castbar_custom then _G.pfSkin_throttle.nameplates_castbar_custom = "50" end
    if not _G.pfSkin_throttle.nameplates_mass_custom then _G.pfSkin_throttle.nameplates_mass_custom = "7" end
    if not _G.pfSkin_throttle.tooltip_cursor_custom then _G.pfSkin_throttle.tooltip_cursor_custom = "10" end
    if not _G.pfSkin_throttle.chat_tab_custom then _G.pfSkin_throttle.chat_tab_custom = "10" end
    if not _G.pfSkin_throttle.swingtimer_custom then _G.pfSkin_throttle.swingtimer_custom = "50" end
...
-- Export to pfSkin namespace for easier access
pfSkin.throttle = libthrottle
