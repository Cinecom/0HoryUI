...
function pfSkin.api.HasSuperWoW()
...
function pfSkin.api.HasNampower()
...
function pfSkin.api.GetUnitDistance(unit1, unit2)
  if not unit2 then
    unit2 = unit1
    unit1 = "player"
  end
  ...
  if pfSkin.api.HasSuperWoW() and UnitPosition then
...
-- Requires UnitXP_SP3
function pfSkin.api.UnitInLineOfSight(unit1, unit2)
  if not pfSkin.api.HasUnitXP() then return nil end
...
-- Requires UnitXP_SP3
function pfSkin.api.UnitIsBehind(unit1, unit2)
  if not pfSkin.api.HasUnitXP() then return nil end
...
function pfSkin.api.strsplit(delimiter, subject)
...
function pfSkin.api.isempty(tbl)
...
function pfSkin.api.checkversion(chkmajor, chkminor, chkfix)
  if not major and not minor and not fix then
    -- load and convert current version
    major, minor, fix = pfSkin.api.strsplit(".", tostring(pfSkin_config.version))
...
local RangeCache = {}
function pfSkin.api.UnitInRange(unit)
  ...
  if pfSkin.api.librange then
    return pfSkin.api.librange:UnitInSpellRange(unit)
  end
  return nil
end
...
local queue, frame = {}
function pfSkin.api.RunOOC(func)
  if not frame then
...
function pfSkin.api.UnitHasBuff(unit, buff)
  local hasbuff = nil
  for i=1,32 do
...
function pfSkin.api.GetUnitColor(unitstr)
  ...
  return pfSkin.api.rgbhex(r,g,b), r, g, b
end
...
function pfSkin.api.strvertical(str)
...
function pfSkin.api.round(input, places)
...
function pfSkin.api.clamp(x, min, max)
...
function pfSkin.api.GetSlashCommands(text)
  local cmds
...
function pfSkin.api.RegisterSlashCommand(name, cmds, func, force)
  local counter = 1

  for _, cmd in pairs(cmds) do
    if force or not pfSkin.api.GetSlashCommands(cmd) then
...
local capture_cache = {}
function pfSkin.api.GetCaptures(pat)
...
function pfSkin.api.SanitizePattern(pattern)
...
function pfSkin.api.cmatch(str, pat)
  -- read capture indexes
  a, b, c, d, e = GetCaptures(pat)
  _, _, va, vb, vc, vd, ve = string.find(str, pfSkin.api.SanitizePattern(pat))
...
function pfSkin.api.GetItemLinkByName(name)
...
function pfSkin.api.GetItemCount(itemName)
  local count = 0
  for bag = 4, 0, -1 do
...
-- returns:     [int]            bag
--              [int]            slot
function pfSkin.api.FindItem(item)
  for bag = 4, 0, -1 do
    for slot = 1, GetContainerNumSlots(bag) do
      local itemLink = GetContainerItemLink(bag,slot)
      if itemLink then
...
function pfSkin.api.GetBagFamily(bag)
...
function pfSkin.api.Abbreviate(number)
  local mode = pfSkin_config.unitframes.abbrevnum
  ...
  if mode == "1" or mode == "2" then
    ...
    if number > 1000000 then
      if mode == "2" then
        ...
        return pfSkin.api.round(number/1000000*sign, 2) .. "m"
      end
    elseif number > 1000 then
      if mode == "2" then
        ...
        return pfSkin.api.round(number/1000*sign, 2) .. "k"
      end
    end
  end
...
function pfSkin.api.SendChatMessageWide(msg)
  local channel = "SAY"
...
do -- create a scope so we don't have to worry about upvalue collisions
  ...
  function pfSkin.api.GroupInfoByName(name,group)
    unitinfo = pfSkin.api.wipe(unitinfo)
    if group == "party" then
...
local timer
function pfSkin.api.QueueFunction(a1,a2,a3,a4,a5,a6,a7,a8,a9)
  if not timer then
...
function pfSkin.api.CreateGoldString(money)
...
function pfSkin.api.EnableMovable(name, addon, blacklist)
  if addon then
...
function pfSkin.api.CopyTable(src)
  local lookup_table = {}
...
function pfSkin.api.LoadMovable(frame, init)
  ...
  if pfSkin_config["position"][frame:GetName()] then
    if pfSkin_config["position"][frame:GetName()]["parent"] then
      frame:SetParent(_G[pfSkin_config["position"][frame:GetName()]["parent"]])
    end

    if pfSkin_config["position"][frame:GetName()]["scale"] then
      frame:SetScale(pfSkin_config["position"][frame:GetName()].scale)
    end

    if pfSkin_config["position"][frame:GetName()]["xpos"] then
      local anchor = pfSkin_config["position"][frame:GetName()]["anchor"] or "TOPLEFT"
      frame:ClearAllPoints()
      frame:SetPoint(anchor, pfSkin_config["position"][frame:GetName()].xpos, pfSkin_config["position"][frame:GetName()].ypos)
    end
...
function pfSkin.api.SaveMovable(frame, scale)
...
-- 'init'       [bool]         treats the current position as initial data
function pfSkin.api.UpdateMovable(frame, init)
  local name = frame:GetName()

  if pfSkin_config.global.offscreen == "0" then
    frame:SetClampedToScreen(true)
  end

  if not pfSkin.movables[name] then
    pfSkin.movables[name] = frame
  end
...
function pfSkin.api.RemoveMovable(frame)
  local name = frame:GetName()
  pfSkin.movables[name] = nil
end
...
function pfSkin.api.AlignToPosition(frame, anchor, position, spacing)
...
function pfSkin.api.SetAutoPoint(frame, parent, spacing)
  --[[
...
function pfSkin.api.GetBestAnchor(self)
...
function pfSkin.api.ConvertFrameAnchor(self, anchor)
...
-- [ GetStringColor ]
-- Queries the pfSkin setting strings and extract its color codes
-- returns r,g,b,a
local color_cache = {}
function pfSkin.api.GetStringColor(colorstr)
  if not color_cache[colorstr] then
    local r, g, b, a = pfSkin.api.strsplit(",", colorstr)
...
local _r, _g, _b, _a
function pfSkin.api.rgbhex(r, g, b, a)
...
function pfSkin.api.GetBorderSize(pref)
  if not pfSkin.borders then pfSkin.borders = {} end

  -- set to default border if accessing a wrong border type
  if not pref or not pfSkin_config.appearance.border[pref] or pfSkin_config.appearance.border[pref] == "-1" then
    pref = "default"
  end

  if pfSkin.borders[pref] then
    -- return already cached values
    return pfSkin.borders[pref][1], pfSkin.borders[pref][2]
  else
    -- add new borders to the pfSkin tree
    local raw = tonumber(pfSkin_config.appearance.border[pref])
    if raw == -1 then raw = 3 end

    local scaled = raw * GetPerfectPixel()
    pfSkin.borders[pref] = { raw, scaled }
...
function pfSkin.api.GetPerfectPixel()
  if pfSkin.pixel then return pfSkin.pixel end

  if pfSkin_config.appearance.border.pixelperfect == "1" then
    local scale = GetCVar("useUiScale") == "1" and GetCVar("uiScale") or "1"
    local resolution = GetCVar("gxResolution")
    local _, _, screenwidth, screenheight = strfind(resolution, "(.+)x(.+)")

    pfSkin.pixel = 768 / screenheight / scale
    pfSkin.pixel = pfSkin.pixel > 1 and 1 or pfSkin.pixel

    -- autodetect and zoom for HiDPI displays
    if pfSkin_config.appearance.border.hidpi == "1" then
      pfSkin.pixel = pfSkin.pixel < .5 and pfSkin.pixel * 2 or pfSkin.pixel
    end
  else
    pfSkin.pixel = .7
  end
  ...
  pfSkin.backdrop = {
    ...
    edgeFile = "Interface\\BUTTONS\\WHITE8X8", edgeSize = pfSkin.pixel,
    insets = {left = -pfSkin.pixel, right = -pfSkin.pixel, top = -pfSkin.pixel, bottom = -pfSkin.pixel},
  }

  pfSkin.backdrop_thin = {
    bgFile = "Interface\\BUTTONS\\WHITE8X8", tile = false, tileSize = 0,
    edgeFile = "Interface\\BUTTONS\\WHITE8X8", edgeSize = pfSkin.pixel,
...
-- [ Create Backdrop ]
-- Creates a pfSkin compatible frame as backdrop element
...
function pfSkin.api.CreateBackdrop(f, inset, legacy, transp, backdropSetting)
  ...
  backdrop = blizz and pfSkin.backdrop_blizz_full or rawborder == 1 and pfSkin.backdrop_thin or pfSkin.backdrop
  border = blizz and math.max(border, 3) or border

  -- get the color settings
  br, bg, bb, ba = pfSkin.api.GetStringColor(pfSkin_config.appearance.border.background)
  er, eg, eb, ea = pfSkin.api.GetStringColor(pfSkin_config.appearance.border.color)
  ...
  if legacy then
    ...
    if blizz then
      ...
      f.backdrop_border:SetBackdrop(pfSkin.backdrop_blizz_border)
...
-- [ Create Shadow ]
-- Creates a pfSkin compatible frame as shadow element
-- 'f'          [frame]         the frame which should get a backdrop.
function pfSkin.api.CreateBackdropShadow(f)
  -- exit if now frame was given
  if not f then return end

  if f.backdrop_shadow or pfSkin_config.appearance.border.shadow ~= "1" then
    return
  end
  ...
  f.backdrop_shadow:SetBackdrop(pfSkin.backdrop_shadow)
  f.backdrop_shadow:SetBackdropBorderColor(0, 0, 0, tonumber(pfSkin_config.appearance.border.shadow_intensity))
end

-- [ Bar Layout Options ] --
-- 'barsize'  size of bar in number of buttons
-- returns:   array of options as strings for pfSkin.gui.bar
function pfSkin.api.BarLayoutOptions(barsize)
...
local function ResolveBarLayout(barsize, formfactor, uneven)
  ...
  local index = pfSkin.api.BarLayoutFormfactor(layout)
...
function pfSkin.api.BarLayoutSize(bar,barsize,formfactor,iconsize,bordersize,padding,uneven,fillmode)
...
-- 'padding'      the spacing between buttons
function pfSkin.api.BarButtonAnchor(button,basename,buttonindex,barsize,formfactor,iconsize,bordersize,padding,uneven,fillmode)
...
function pfSkin.api.EnableAutohide(frame, timeout, combat)
...
function pfSkin.api.DisableAutohide(frame)
  if not frame then return end
...
function pfSkin.api.GetColoredTimeString(remaining)
  if not remaining then return "" end

  -- Show days if remaining is > 99 Hours (99 * 60 * 60)
  if remaining > 356400 then
    if not color_day then
      local r,g,b,a = pfSkin.api.GetStringColor(C.appearance.cd.daycolor)
      color_day = pfSkin.api.rgbhex(r,g,b)
    end
  ...
  elseif remaining > 5940 then
    if not color_hour then
      local r,g,b,a = pfSkin.api.GetStringColor(C.appearance.cd.hourcolor)
      color_hour = pfSkin.api.rgbhex(r,g,b)
    end
  ...
  elseif remaining > 99 then
    if not color_minute then
      local r,g,b,a = pfSkin.api.GetStringColor(C.appearance.cd.minutecolor)
      color_minute = pfSkin.api.rgbhex(r,g,b)
    end

    return color_minute .. round(remaining / 60) .. "|rm"

  -- Show milliseconds on low
  elseif remaining <= 5 and pfSkin_config.appearance.cd.milliseconds == "1" then
    if not color_low then
      local r,g,b,a = pfSkin.api.GetStringColor(C.appearance.cd.lowcolor)
      color_low = pfSkin.api.rgbhex(r,g,b)
    end
  ...
  elseif remaining >= 0 then
    if not color_normal then
      local r, g, b, a = pfSkin.api.GetStringColor(C.appearance.cd.normalcolor)
      color_normal = pfSkin.api.rgbhex(r,g,b)
    end
...
function pfSkin.api.GetColorGradient(perc)
  ...
  if not gradientcolors[index] then
    ...
    local h = pfSkin.api.rgbhex(r,g,b)
...
-- 'arg1'       [string]
-- return object
function pfSkin.api.GetNoNameObject(frame, objtype, layer, arg1, arg2)
...
local memoizedFuncs = {}
function pfSkin.api.TryMemoizedFuncLoadstringForSpellCasts(funcOrStr)
